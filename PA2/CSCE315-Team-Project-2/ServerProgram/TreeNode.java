...
public class TreeNode {
	...
	public Board move(int index, Board parentBoard) {
		if(isValidMove(index)) {
			...
			Board childBoard = new Board(parentBoard,1);
			int lastIndex;
	...
	public void generateChildren(int p) {
		...
		if(boardState.totalMovesMade == 1){
			Board pieBoard = new Board(boardState);
			pieBoard.pieMove();
			TreeNode newNode = new TreeNode(pieBoard);
			newNode.currPlayer = nextPlayer;
			newNode.moveString = "P";
			addChild(newNode);
			numChildren++;
		}
	}
